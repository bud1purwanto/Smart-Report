    ...
    <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-950 overflow-hidden select-none">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-extrabold text-sm text-slate-800 dark:text-slate-100">Auto-Blast Telegram & Scheduler</h2>
              ...
              {schedules.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                  <td className="p-3 font-semibold text-slate-800 dark:text-slate-200">
                    <div>{s.name}</div>
                    <div className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">Query ID: #{s.query_id}</div>
                  </td>
                  <td className="p-3 font-mono text-sky-700 dark:text-sky-400 font-bold">
                    {s.cron_expression}
                  </td>
                  <td className="p-3 text-slate-700 dark:text-slate-300">
                    ...
                    {s.telegram_chat_id && (
                      <div className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">Chat ID: {s.telegram_chat_id}</div>
                    )}
                  </td>
                  <td className="p-3">
                    <div className="flex items-center gap-1 text-[10px]">
                      {s.anonymize && (
                        <span className="bg-amber-100 dark:bg-amber-950/50 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-700 px-1.5 py-0.2 rounded-md font-mono font-bold">
                          MASKED
                        </span>
                      )}
                      {s.deduplicate && (
                        <span className="bg-sky-100 dark:bg-sky-950/50 text-sky-800 dark:text-sky-300 border border-sky-300 dark:border-sky-700 px-1.5 py-0.2 rounded-md font-mono font-bold">
                          DEDUP
                        </span>
                      )}
                      <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-1.5 py-0.2 rounded-md font-mono uppercase font-bold">
                        {s.export_format}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="p-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-sky-50 dark:bg-sky-950/50 hover:bg-sky-100 dark:hover:bg-sky-900/50 border border-sky-200 dark:border-sky-800 text-sky-700 dark:text-sky-300 text-[11px] font-semibold flex items-center gap-1 transition"
                      >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl animate-in fade-in zoom-in-95 duration-150 text-slate-800 dark:text-slate-100">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Pilih Query Laporan:</label>
                  <select
                    value={queryId}
                    onChange={(e) => setQueryId(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-slate-800 dark:text-slate-100 focus:outline-none focus:border-sky-500 cursor-pointer"
                  >
                    {queries.map((q) => (
                      <option key={q.id} value={q.id} className="dark:bg-slate-900 dark:text-slate-100">
                        {q.name}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Target Server SAP:</label>
                  <select
                    value={serverId}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 font-mono focus:outline-none focus:border-sky-500"
                />
                <span className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 block">
                  Contoh: <code className="text-sky-600 dark:text-sky-400 font-bold">0 8 * * *</code> = Jam 8 pagi setiap hari, <code className="text-sky-600 dark:text-sky-400 font-bold">0 */4 * * *</code> = Setiap 4 jam.
                </span>
              </div>

              <div>
                <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Telegram Chat ID Penerima:</label>
                <input
                  type="text"
              ...
              <div className="flex gap-4 pt-1">
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none text-slate-700 dark:text-slate-300">
                  <input
                    ...
                    className="rounded border-slate-300 dark:border-slate-600 text-sky-500 focus:ring-0"
                  />
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
              <button
                onClick={() => setIsModalOpen(false)}
                className="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs hover:bg-slate-200 dark:hover:bg-slate-700 font-semibold transition"
              >
                Batal
              </button>
              <button

      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-5 space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-100">Tambah Kolom Kalkulasi Dinamis (Custom Formula)</h3>
          </div>
          <button
        ...
        <div className="space-y-3 text-xs">
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Nama Kolom Baru:</label>
            <input
              type="text"
              value={columnName}
              onChange={(e) => setColumnName(e.target.value)}
              placeholder="Contoh: TOTAL_AMOUNT, PPN_11, MARGIN_PCT..."
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 uppercase font-mono focus:outline-none focus:border-indigo-500"
            />
          </div>

          <div>
            <label className="block font-semibold text-slate-300 mb-1">Formula Ekspresi (Javascript / Pandas):</label>
            <textarea
              rows={3}
              value={formula}
              onChange={(e) => setFormula(e.target.value)}
              placeholder="Contoh: row.MENGE * row.NETPR  atau  row.NETPR * 0.11"
              className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 text-slate-100 font-mono focus:outline-none focus:border-indigo-500"
            />
            <p className="text-[11px] text-slate-500 mt-1">
              Gunakan sintaks <code className="text-indigo-300">row.NamaKolom</code> untuk mengakses nilai baris data.
            </p>
          </div>

          {/* Quick Insert Tokens */}
          <div>
            <span className="text-[11px] text-slate-400 font-medium block mb-1.5">Klik kolom untuk memasukkan ke formula:</span>
            <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
              {columns.map((c) => (
                <button
                  ...
                  className="px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 border border-slate-700/80 text-sky-400 font-mono text-[10px] transition"
                >
                  +{c}
                </button>
              ))}
